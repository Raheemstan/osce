...
<div class="content content-dark">
  ...
  <div class="row" style="width:99%">
    ...
    <div class="col-md-4">
      ...
      <?php
      if (post('exam_date')) {
        $date_input = strtotime(filter_input(INPUT_POST, 'exam_date', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
        if ($date_input) {
          $squery = mysqli_query($conn, "UPDATE user_info SET exam_date='$date_input' WHERE email='$email'");
          if ($squery) {
            $exam_date = $date_input;
          }
        }
      } else {
        $query = mysqli_query($conn, "SELECT exam_date FROM user_info WHERE email='$email'");
        if ($query->num_rows > 0) {
          $exam_date = mysqli_fetch_assoc($query)['exam_date'];
        } else {
          $exam_date = 0;
        }
      }
      ...
      ?>
        ...
        <div style="background: #000;padding: 20px;" id="exam_date">
